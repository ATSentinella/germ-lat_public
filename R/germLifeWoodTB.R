...
#' Longevity and growth form on tolerance breadth 
#+ woodiness-longevity-tolerance-breadth-future, eval = FALSE


getLifeWoodTB <- function() {
  ...
  MSBP <- MSBP %>%
    mutate(vi = SEbreadth*SEbreadth) %>% #Standard error for tolerance breadth
  ...
  MSBP.meta.wood <- mutate(MSBPlong, vi = SEbreadth*SEbreadth) %>%
    filter(woodiness == "W"|woodiness == "H")
  
  #Filter dataset for analysis (with phylogeny)
  MSBP.meta.wood.phy <- mutate(MSBPlong, vi = SEbreadth*SEbreadth) %>%
    filter(woodiness == "W"|woodiness == "H")  %>%
  ...
  ## Germination Tolerance Breadth ~ Woodiness only (with phylogeny)
  wood.mv.TB.phy <- rma.mv(yi = Tbreadth, 
                           V = vi, 
  ...
  ## Germination Tolerance Breadth ~ Woodiness only (without phylogeny)
  wood.mv.TB <- rma.mv(yi = Tbreadth, 
                       V = vi, 
                       mod = ~woodiness + log10(SeedAge) + altitude,
  ...
  woodbox <- ggplot(data = MSBP.meta.wood, aes(x = woodiness, y = Tbreadth)) +
    geom_boxplot(fill = "orchid") + 
    theme_classic()+
    ylab(expression("Germination Tolerance Breadth ",(degree*C)))+
    xlab("") +
    ylim(-45, 10) +
  ...
  #Filter dataset for analysis (with phylogeny)
  MSBP.meta.long <- mutate(MSBPlong, vi = SEbreadth*SEbreadth) %>%
    filter(ann_per == "annual"|ann_per == "perennial")
  
  #Filter dataset for analysis (without phylogeny)
  MSBP.meta.long.phy <- mutate(MSBPlong, vi = SEbreadth*SEbreadth) %>%
    filter(ann_per == "annual"|ann_per == "perennial")%>%
    distinct(Taxon_ID, .keep_all = T) # needed for phlogentic analysis
  ...
  ## Germination Tolerance Breadth  ~ Longevity only (with phylogeny)
  long.mv.TB.phy <- rma.mv(yi = Tbreadth, 
                           V = vi, 
  ...
  ## Germination Tolerance Breadth  ~ Longevity only (without phylogeny)
  long.mv.TB <- rma.mv(yi = Tmax - Tmin, 
                       V = vi, 
                       mod = ~ann_per  + log10(SeedAge) + altitude,
                       random = list(~1 | Taxon_ID, ~1 | grid.ll),
                       method = "REML", 
  ...
  #Boxplot comparing woody and herbaceous species
  longbox <- ggplot(data = MSBP.meta.long, aes(x = ann_per, y = Tmax - Tmin)) +
    geom_boxplot(fill = "orchid") + 
    theme_classic()+
    ylab(expression("Germination Tolerance Breadth ",(degree*C)))+
    xlab("") +
  ...
  output <- list(Wood.Model = wood.mv.TB, 
                 Wood.Model.Phy = wood.mv.TB.phy, 
                 Wood.Plot = woodbox,
                 Long.Model = long.mv.TB,
                 Long.Model.Phy = long.mv.TB.phy,
                 Long.Plot = longbox)
